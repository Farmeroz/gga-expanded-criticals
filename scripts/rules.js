...
export function validSettings(settings) {
  return (
    ...
    Number.isSafeInteger(settings.firstExtraSkill) &&
    settings.firstExtraSkill >= 17 &&
    Number.isSafeInteger(settings.interval) &&
    settings.interval >= 1
  );
}
...
export function criticalThreshold(effectiveSkill, settings = DEFAULTS) {
  ...
  return Math.min(
    16,
    7 + Math.floor((effectiveSkill - settings.firstExtraSkill) / settings.interval),
  );
}
...
export function expandCritical(chatdata, settings = DEFAULTS) {
  if (!validSettings(settings) || !settings.enabled || !chatdata || typeof chatdata !== 'object')
    return false;
  if (
    !['number', 'string'].includes(typeof chatdata.origtarget) ||
    !Number.isFinite(Number(chatdata.origtarget)) ||
    Number(chatdata.origtarget) <= 0 ||
    !Number.isFinite(chatdata.finaltarget) ||
    !Number.isInteger(chatdata.rtotal) ||
    chatdata.rtotal < 7 ||
    chatdata.rtotal > 16 ||
    chatdata.isCritSuccess !== false ||
    chatdata.isCritFailure !== false ||
    chatdata.failure !== false ||
    chatdata.seventeen !== false ||
    chatdata.rtotal > chatdata.finaltarget
  )
    return false;
