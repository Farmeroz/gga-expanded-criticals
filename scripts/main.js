...
Hooks.once('init', () => {
  ...
  game.settings.register(ID, 'enabled', {
    ...common,
    name: 'Enable expanded critical successes',
    type: Boolean,
  ...
  game.settings.register(ID, 'firstExtraSkill', {
    ...common,
    name: 'First expanded critical at effective skill',
  ...
  if (Number(game.release.generation) !== 14 || !/^0\.18\.\d+$/.test(game.system.version)) {
    ...
    status = installIntegration(globalThis.GURPS, readSettings, () => {
      ui.notifications.warn(
        "GGA Expanded Criticals could not apply the house rule. GGA's normal result was retained. Check the module settings.",
      );
    });
  }
