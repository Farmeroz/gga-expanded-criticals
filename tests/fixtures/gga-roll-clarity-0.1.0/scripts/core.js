...
export function authorId(message) {
  return idOf(message.author) ?? idOf(message.user) ??
    idOf(message._source?.author) ?? idOf(message._source?.user);
}
...
export function modeOf(message, users) {
  ...
  if (ids.some(id => users.get(id)?.isGM) &&
      ids.every(id => id === authorId(message) || users.get(id)?.isGM)) return "gm";
  return "custom";
}

export function canReceipt(message, creatorId, viewer, users) {
  return !!viewer && !viewer.isGM && viewer.id === creatorId &&
    authorId(message) === creatorId && hasRoll(message) &&
    !message.flags?.[ID]?.receipt && modeOf(message, users) === "blind";
}
...
export function labelFromOTF(value) {
  ...
  const attribute = /^(FRIGHT\s?CHECK|TASTE\s?SMELL|VISION|HEARING|TOUCH|SMELL|TASTE|DODGE|PARRY|BLOCK|WILL|PER|ST|DX|IQ|HT|QN)(?=$|[\s\d+−–\-?:])/i.exec(text);
...
export function describeCheck(content, Parser = globalThis.DOMParser) {
  ...
  const command = heading.querySelector("[data-otf]")?.getAttribute("data-otf") ?? heading.textContent;
...
export function makeReceipt({ sourceId, rollerId, character, check, baseTarget, audience, detail, gmIds }) {
  const name = escapeHTML(safeText(character) || "Your character");
  const subject = detail !== "generic" && check ? ` against <strong>${escapeHTML(safeText(check))}</strong>` : "";
  const base = subject && detail === "base" && Number.isInteger(baseTarget) && baseTarget > 0
    ? ` (original target ${baseTarget}, before roll modifiers)` : "";
  const whisper = audience === "public" ? [] : audience === "gm"
    ? [...new Set([rollerId, ...gmIds])] : [rollerId];
  return {
    author: rollerId,
...
export function decorate(message, html, { users, showLabels = true, borders = true }) {
  ...
  for (const cls of [...html.classList]) if (cls.startsWith("grc-mode-")) html.classList.remove(cls);
  if (!showLabels || message.visible === false || message.isContentVisible === false ||
      !hasRoll(message) || message.flags?.[ID]?.receipt) return;
...
export class ReceiptController {
  constructor({ user, users, settings, describe = describeCheck, randomId, create, notify, now = Date.now }) {
  ...
  before(message, _data, _options, creatorId) {
    ...
    if (!user || user.isGM || creatorId !== user.id || authorId(message) !== user.id ||
        !hasRoll(message) || message.flags?.[ID]?.receipt || settings.audience === "off") return;
    for (const [key, entry] of this.pending) if (this.now() - entry.created > 600_000) this.pending.delete(key);
    const token = this.randomId();
    const description = settings.detail === "generic" ? { check: null, baseTarget: null } : this.describe(message.content);
    this.pending.set(token, {
      ...description, character: safeText(message.speaker?.alias) || safeText(user.name) || "Your character",
      created: this.now(), audience: settings.audience, detail: settings.detail
    });
  ...
  async after(message, _options, creatorId) {
    ...
    if (!canReceipt(message, creatorId, this.user(), this.users()) || this.settings().audience === "off") return;
    const users = this.users();
    const receipt = makeReceipt({ ...pending, sourceId: message.id, rollerId: creatorId,
    ...
    try {
      ...
      const created = await this.create(receipt, {
        messageMode: pending.audience === "public" ? "public" : pending.audience === "gm" ? "gm" : "self"
      });
      if (!created) throw new Error("Receipt creation cancelled");
    } catch {
      this.notify("The blind roll was created, but its confirmation could not be saved. Please ask the GM to confirm it.");
    }
  }
}
